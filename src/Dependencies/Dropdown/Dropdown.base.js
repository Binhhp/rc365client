...
} from "../@uifabric/utilities/selectableOption";
...
var DropdownBase = /** @class */ (function (_super) {
  ...
  function DropdownBase(props) {
    ...
    _this._onChange = function (event, options, index, checked, multiSelect) {
      ...
      if (onChange || onChanged) {
        ...
        var changedOpt = multiSelect
          ? __assign(__assign({}, options[index]), { selected: !checked })
          : options[index];
        onChange &&
          onChange(
            __assign(__assign({}, event), { target: _this._dropDown.current }),
            changedOpt,
            index
          );
        ...
        : React.createElement(
            ...
            __assign(
              ...
              calloutProps,
              {
                ...
                calloutWidth:
                  dropdownWidth ||
                  (_this._dropDown.current
                    ? _this._dropDown.current.clientWidth
                    : 0),
              }
            ),
            _this._renderFocusableList(props)
      ...
      var emptyQueue = function () {
        var newGroup = queue.id
          ? [
              React.createElement(
                "div",
                { role: "group", key: queue.id, "aria-labelledby": queue.id },
                queue.items
              ),
            ]
          : queue.items;
      ...
      var placeRenderedOptionIntoQueue = function (item, index) {
        ...
        switch (item.itemType) {
          case SelectableOptionMenuItemType.Header:
            ...
            queue.items.push(
              onRenderItem(
                __assign(__assign({ id: id }, item), { index: index }),
                _this._onRenderItem
              )
            );
            queue.id = id;
          ...
          case SelectableOptionMenuItemType.Divider:
            index > 0 &&
              queue.items.push(
                onRenderItem(
                  __assign(__assign({}, item), { index: index }),
                  _this._onRenderItem
                )
              );
            queue.items.length > 0 && emptyQueue();
      ...
      return !_this.props.multiSelect
        ? React.createElement(
            CommandButton,
            {
              ...
              rcName: rcName
                ? `ddl.Opt.${usingRcNameWithText ? item.text : item.key}`
                : undefined,
            },
        ...
        : React.createElement(CustomCheckBox, {
            ...
            darkMode,
            rcName: rcName
              ? `ddl.${rcName}.${usingRcNameWithText ? item.text : item.key}`
              : undefined,
          });
    };
    ...
    _this._onRenderOption = function (item) {
      ...
      return React.createElement(
        "span",
        {
          ...
          "data-rc-id": rcName
            ? `ddl.Opt.sp.${rcName}.${
                usingRcNameWithText ? item.text : item.key
              }`
            : undefined,
        },
        item.text
        ...
        _this._requestAnimationFrame(function () {
          ...
          if (_this._focusZone.current) {
            if (
              ...
              var element = getDocument().getElementById(
                _this._id + "-list" + selectedIndices[0]
              );
              if (element) {
        ...
        case KeyCodes.down:
          if (!containsExpandCollapseModifier && _this._host.current) {
            elementToFocus = getFirstFocusable(
              _this._host.current,
              _this._host.current.firstChild,
              true
            );
          }
          break;
      ...
      if (multiSelect) {
        var warnMultiSelect = function (prop) {
          return warn(
            "Dropdown property '" +
              prop +
              "' cannot be used when 'multiSelect' is true. Use '" +
              prop +
              "s' instead."
          );
        };
        ...
        var warnNotMultiSelect = function (prop) {
          return warn(
            "Dropdown property '" +
              prop +
              "s' cannot be used when 'multiSelect' is false/unset. Use '" +
              prop +
              "' instead."
          );
        };
    ...
    var selectedIndices;
    if (multiSelect) {
      selectedIndices = _this._getSelectedIndexes(
        options,
  ...
  DropdownBase.prototype.UNSAFE_componentWillReceiveProps = function (
    newProps
  ) {
  ...
  DropdownBase.prototype.render = function () {
    ...
    return React.createElement(
          ...
          return React.createElement(
            ...
            __assign(
              ...
              keytipAttributes,
              {
                ...
                "aria-labelledby":
                  label && !ariaLabel
                    ? mergeAriaAttributeValues(_this._labelId, _this._optionId)
                    : undefined,
                "aria-describedby": mergeAriaAttributeValues(
                  keytipAttributes["aria-describedby"],
                  hasErrorMessage ? _this._id + "-errorMessage" : undefined
                ),
                "aria-activedescendant": ariaActiveDescendant,
                "aria-required": required,
                "aria-disabled": disabled,
                "aria-owns": isOpen ? _this._listId : undefined,
                "data-rc-id": rcName ? `ddl.${rcName}` : undefined,
              },
              divProps,
              {
            ...
            React.createElement(
              "span",
              {
                ...
                "data-rc-id": rcName ? `ddl.sp.${rcName}` : undefined,
      ...
      isOpen &&
        onRenderContainer(
          __assign(__assign({}, props), { onDismiss: this._onDismiss }),
      ...
      React.createElement(
        "div",
        {
          ...
          "data-rc-id": rcName ? `errMsg.${rcName}` : undefined,
        },
    ...
    return React.createElement(
      ...
      React.createElement(
        FocusZone,
        {
          ...
          "data-rc-id": rcName ? `ddt.Opt.List.${rcName}` : undefined,
        },
  ...
  DropdownBase.prototype._renderHeader = function (item) {
    ...
    return React.createElement(
      "div",
      { id: id, key: key, className: this._classNames.dropdownItemHeader },
  ...
  DropdownBase.prototype._getSelectedIndexes = function (options, selectedKey) {
    ...
    var selectedIndices = [];
    for (
      var _i = 0, selectedKey_1 = selectedKey;
      _i < selectedKey_1.length;
      _i++
    ) {
