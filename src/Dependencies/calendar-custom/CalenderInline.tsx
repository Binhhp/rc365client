...
class CalendarInline extends React.Component<ICalendarInlineExampleProps, any> {
  ...
  onGetMode = async () => {
    ...
    this.props.onGetSelectMode &&
      this.props.onGetSelectMode(this.state.pickType);
    this.sentDate();
  };

  onHighLightSelectionRange = async (startDate: Date, endDate: Date) => {
    let dateRange = [];
  ...
  render() {
    ...
    let CalendarWrapper = {
      [nameAttibute]: `cal.${this.props.rcName}`,
    };
    return (
      <div className="CalendarWrapper" {...CalendarWrapper} style={divStyle}>
